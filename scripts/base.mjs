...
export class Autostake {
  ...
  async run(networkNames) {
    ...
    for (const name of networkNames) {
      if (name && !networks.map((el) => el.name).includes(name))
        return timeStamp("Invalid network name:", name);
    }
    const calls = networks.map((data) => {
      return async () => {
        if (
          networkNames &&
          networkNames.length &&
          !networkNames.includes(data.name)
        )
          return;
        ...
        try {
          ...
          return health.failed(
            "Autostake failed, skipping network",
            error.message
          );
        }
      };
  ...
  async runNetwork(client) {
    ...
    timeStamp(
      "Found",
      grantedAddresses.length,
      "delegators with valid grants..."
    );

    let grantMessages = await this.getAutostakeMessages(
      client,
      grantedAddresses
    );
  ...
  async getClient(data, health) {
    ...
    if (network.slip44 && network.slip44 !== slip44) {
      ...
      timeStamp(
        "!! You should switch to the correct path unless you have grants. Check the README !!"
      );
    }
  ...
  checkBalance(client) {
    return client.queryClient
      .getBalance(client.operator.botAddress, client.network.denom)
      .then(
        (balance) => {
  ...
  async getGrantedAddresses(client, addresses) {
    ...
    let grantedAddresses = await mapSync(
      grantCalls,
      batchSize,
      (batch, index) => {
  ...
  getGrants(client, delegatorAddress) {
    ...
    return client.queryClient
      .getGrants(client.operator.botAddress, delegatorAddress, { timeout })
      .then(
        (result) => {
          if (result.stakeGrant) {
            if (
              result.stakeGrant.authorization["@type"] ===
              "/cosmos.authz.v1beta1.GenericAuthorization"
            ) {
              timeStamp(
                delegatorAddress,
                "Using GenericAuthorization, allowed"
              );
              return [client.operator.address];
            }

            const grantValidators =
              result.stakeGrant.authorization.allow_list.address;
            if (!grantValidators.includes(client.operator.address)) {
              timeStamp(
                delegatorAddress,
                "Not autostaking for this validator, skipping"
              );
              return;
            }
  ...
  async getAutostakeMessage(client, grantAddress) {
    ...
    if (
      smaller(
        bignumber(autostakeAmount),
        bignumber(client.operator.minimumReward)
      )
    ) {
      timeStamp(
        address,
        autostakeAmount,
        client.network.denom,
        "reward is too low, skipping"
      );
      return;
    }
    ...
    if (grant.maxTokens) {
      if (smallerEq(grant.maxTokens, 0)) {
        timeStamp(
          ...
          grant.maxTokens,
          client.network.denom,
          "grant balance is empty, skipping"
        );
        return;
      }
      if (smaller(grant.maxTokens, autostakeAmount)) {
        ...
        timeStamp(
          ...
          grant.maxTokens,
          client.network.denom,
          "grant balance is too low, using remaining"
        );
      }
    }

    let timeout = client.network.data.autostake?.delegatorTimeout || 5000;
    const withdrawAddress = await client.queryClient.getWithdrawAddress(
      address,
      { timeout }
    );
    ...
    let messages = this.buildRestakeMessage(
      address,
      client.operator.address,
      autostakeAmount,
      client.network.denom
    ...
    let calls = batches.map((batch, index) => {
      return async () => {
        try {
          ...
          const gas = await client.signingClient.simulate(
            client.operator.botAddress,
            batch,
            memo,
            gasModifier
          );

          const defaultFee = client.network.data.autostake?.defaultFee;
          console.log(
            "gasModifier",
            gasModifier,
            "gas",
  ...
  totalRewards(client, address) {
    let timeout = client.network.data.autostake?.delegatorTimeout || 5000;
    return client.queryClient.getRewards(address, { timeout }).then(
      (rewards) => {
        const total = Object.values(rewards).reduce((sum, item) => {
          const reward = item.reward.find(
            (el) => el.denom === client.network.denom
          );
  ...
  getNetworksData() {
    ...
    try {
      ...
      timeStamp(
        "Failed to parse networks.local.json, check JSON is valid",
        error.message
      );
      return networks;
    }
