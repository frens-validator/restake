...
class Network {
  ...
  connectedDirectory() {
    const apis = this.chain
      ? this.chain.chainData["best_apis"]
      : this.data["best_apis"];
  ...
  async load() {
    ...
    this.operators = (
      this.data.operators || this.validators.filter((el) => el.restake)
    ).map((el) => {
      return Operator(el);
    });
    ...
    const defaultGasPrice =
      format(bignumber(multiply(0.000000025, pow(10, this.decimals))), {
        notation: "fixed",
      }) + this.denom;
  ...
  async connect() {
    try {
      ...
      this.connected =
        this.queryClient.connected &&
        (!this.usingDirectory || this.connectedDirectory());
    } catch (error) {
      console.log(error);
  ...
  async getApy(validators, operators) {
    ...
    for (const [address, validator] of Object.entries(validators)) {
      if (validator.jailed || validator.status !== "BOND_STATUS_BONDED") {
        ...
        const periodPerYear =
          operator && this.chain.authzSupport ? operator.runsPerDay() * 365 : 1;
  ...
  getOperatorByBotAddress(botAddress) {
    console.log(this.operators.find((a) => a.moniker.includes("Frens")));
  ...
  sortOperators() {
    ...
    if (this.data.ownerAddress) {
      return _.sortBy(random, ({ address }) =>
        address === this.data.ownerAddress ? 0 : 1
      );
    }
    return random;
  }
