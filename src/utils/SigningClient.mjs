...
async function SigningClient(network, defaultGasPrice, signer, key) {
  ...
  function getFee(gas, gasPrice, defaultAmount) {
    if (!gas) gas = 200000;
    return calculateFee(gas, gasPrice || defaultGasPrice, defaultAmount);
  }

  async function signAndBroadcastWithoutBalanceCheck(
    address,
    msgs,
    gas,
    memo,
  ...
  async function signAndBroadcast(
    ...
    if (!gas) gas = await simulate(address, messages, memo);
    const fee = getFee(gas, gasPrice, defaultFee);
